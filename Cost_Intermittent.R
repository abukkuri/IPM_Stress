...
for (i in 1:sim_steps) {
  ...
  if (i>=48 && i>=pausetx+4 && params$txon==FALSE){starttx=i
  params$txon=TRUE} 
  
  if ((params$txon==TRUE && i>=starttx+8)){params$txon=FALSE
  pausetx=i} 

  if ((total_cells>popsize[i-1] && params$txon==TRUE && ti==0 && i>starttx||total_cells<10)){ti=i} 
  
  if ((total_cells>1e6 && i>starttx+3||total_cells<10)){plotstop=i
  break} 
  ...
  b.x = function(x, params) {baser*exp(bex*x+bex1*v)}
  ...
  dbdv <-outer(y, y, function(xp,x,params) {baser*bex1*exp(bex*x + bex1*v)}, params=params)
  
  dkds <-outer(y, y, function(xp,x,params) {g.yx(xp,x,params)},params=params)
  dkdb <-outer(y, y, function(xp,x,params) {f.yx(xp,x,params)},params=params)
  ...
  sensv <- sum(sens*(dkds*dsdv+dkdb*dbdv)) * h ^ 2
